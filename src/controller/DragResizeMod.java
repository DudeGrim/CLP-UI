...
/**
 * founadational code CREDITS TO:https://github.com/varren/JavaFX-Resizable-Draggable-Node OPEN SOURCE CODE.
 *  ************* How to use ************************
 *
...
//            setNodeSize(node, x, y, h, w);
        ...
        private void setNodeSize(Node node, double x, double y, double h, double w) {
            ...
            if (node instanceof Canvas) {
                ...
                ((Button) node).setPrefWidth(w);
                ((Button) node).setPrefHeight(h);
            }
            else if (node instanceof Label) {
            	((Label) node).getFont().getSize();
            	if(((Label) node).getPrefWidth()<w||((Label) node).getPrefHeight()<h )
            	{
            		double f = ((Label) node).getFont().getSize();
            		f++;
            		Font fontChange= new Font(f);
            		
            		((Label) node).setFont(fontChange);
            	}else{
            		double f = ((Label) node).getFont().getSize();
            		f--;
            		Font fontChange= new Font(f);
                ...
                ((TextField) node).setPrefWidth(w);
                ((TextField) node).setPrefHeight(h);
            }
        }
    };
    ...
    public static Node makeResizable(Node node) {
       return makeResizable(node, null);
    }

    public static Node makeResizable(Node node, OnDragResizeEventListener listener) {
